...
class paymentservice(NWisefinThread):
    ...
    def create_payment(self, payment_obj,user_id,branch_id,vendor_id):
        ...
        paymode = PayMode.objects.using(self._app_schema(ApplicationNamespace.MASTER_SERVICE)).filter(id=payment_obj.get_paymode_id(), entity_id=self._entity_id()).values('name')
        ...
        if not payment_obj.get_id() is None:
            ...
            if  paymode[0]['name']=='DD':
                # checkupdate = False
                payment_update = SupplierPayment.objects.using(self._current_app_schema()).filter(id=payment_obj.get_id(), entity_id=self._entity_id()).update(
                ...
                payment = SupplierPayment.objects.using(self._current_app_schema()).get(id=payment_obj.get_id(), entity_id=self._entity_id())
                ...
                # return error_obj
                try:
                    payment_update = SupplierPayment.objects.using(self._current_app_schema()).filter(id=payment_obj.get_id(), entity_id=self._entity_id()).update(supplier=payment_obj.get_supplier(),
            ...
            try:
                ...
                payment = SupplierPayment.objects.using(self._current_app_schema()).create(supplier=payment_obj.get_supplier(),
                    ...
                    account_type=payment_obj.get_account_type(),account_no=payment_obj.get_account_no(),
                    beneficiary=payment_obj.get_beneficiary(),remarks=payment_obj.get_remarks(),
                    supplierbranch_id=branch_id,created_by=user_id , entity_id=self._entity_id())
    ...
    def fetch_payment_list(self,request, vys_page,user_id,branch_id):
        queue_arr= SupplierPayment.objects.using(self._current_app_schema()).filter(supplierbranch_id=branch_id, entity_id=self._entity_id()).values('id')
        condition = None
        for vendor in queue_arr:
            logger.info(str(vendor))
            if condition is None:
                condition = (Q(id__exact=vendor['id']) & Q(status__exact=1) & (Q(modify_status__exact=-1) |Q(modify_status__exact=0))& Q(modified_by=-1))&Q(entity_id=self._entity_id())
            else:
                condition |= (Q(id__exact=vendor['id']) & Q(status__exact=1) & (Q(modify_status__exact=-1) |Q(modify_status__exact=0))& Q(modified_by=-1))&Q(entity_id=self._entity_id())
    ...
    def fetch_payment(self, payment_id):
        try:
            payment = SupplierPayment.objects.using(self._current_app_schema()).get(id=payment_id, entity_id=self._entity_id())
            payment_data = PaymentResponse()
            payment_data.set_id(payment.id)
            payment_data.set_supplier(payment.supplier)
            payment_data.set_paymode_id(payment.paymode_id)
            ...
            if payment.is_active == 1:
                payment_data.payment_active_status = 'Active'
            else:
                payment_data.payment_active_status = 'Inactive'
            return payment_data
    ...
    def inactive_payment(self, payment_id,vendor_id,user_id,branch_id):
        payment = SupplierPayment.objects.using(self._current_app_schema()).filter(id=payment_id, entity_id=self._entity_id()).update(status=0)
        vendor_service=VendorService(self._scope())
        vendor_check = vendor_service.branchvalidate(branch_id)
        if vendor_check == True:
            vendor_branchupdate = SupplierBranch.objects.using(self._current_app_schema()).filter(id=branch_id, entity_id=self._entity_id()).update(is_validate=True)
    ...
    def payment_modification(self, payment_obj, user_id, branch_id,vendor_id):
        ...
        if not payment_obj.get_id() is None:
               # try:
                   ...
                   if ref_flag==True:
                       payment = SupplierPayment.objects.using(self._current_app_schema()).filter(id=payment_obj.get_id(), entity_id=self._entity_id()).update(supplier=payment_obj.get_supplier(),
                                                                paymode_id=payment_obj.get_paymode_id(),
                                                                bank_id=payment_obj.get_bank_id(),
                                                                branch_id=payment_obj.get_branch_id(),
                                                                account_type=payment_obj.get_account_type(),
                       ...
                       payment = SupplierPayment.objects.using(self._current_app_schema()).create(supplier=payment_obj.get_supplier(),
                                                                ...
                                                                modified_by=user_id, entity_id=self._entity_id())

                       payment_update=SupplierPayment.objects.using(self._current_app_schema()).filter(id=payment_obj.get_id(), entity_id=self._entity_id()).update(modify_ref_id=payment.id)
    ...
    def modification_delete_payment(self, payment_id, vendor_id,user_id,branch_id):
           try:
               vendor_service = VendorService(self._scope())
               payment=SupplierPayment.objects.using(self._current_app_schema()).filter(id=payment_id, entity_id=self._entity_id()).update(modify_ref_id=payment_id,modify_status=ModifyStatus.delete)
               ...
               if ref_flag == True:
                   flag_obj = VendorModificationRel.objects.using(self._current_app_schema()).filter(
                       Q(modify_ref_id=payment_id) & Q(ref_type=VendorRefType.VENDOR_PAYMENT)&Q(entity_id=self._entity_id())).update(
    ...
    def modification_paymentactive(self, payment_obj,branch_id, vendor_id, user_id):
        ...
        if not payment_obj.get_id() is None:
            ref_flag = vendor_service.checkmodify_rel(VendorRefType.VENDOR_PAYMENT, payment_obj.get_id())
            if (ref_flag == True):
                payment = SupplierPayment.objects.using(self._current_app_schema()).filter(id=payment_obj.get_id(), entity_id=self._entity_id()).update(
                    supplier=payment_obj.get_supplier(),
                    paymode_id=payment_obj.get_paymode_id(),
                    bank_id=payment_obj.get_bank_id(),
                    branch_id=payment_obj.get_branch_id(),
    ...
    def modification_action_payment(self, mod_status, old_id, new_id,vendor_id,user_id):
        ...
        if mod_status == ModifyStatus.update:
               ...
               payment_update=SupplierPayment.objects.using(self._current_app_schema()).filter(id=old_id, entity_id=self._entity_id()).update(supplier=payment_obj.get_supplier(),
                                                                ...
                                                                beneficiary=payment_obj.get_beneficiary(),
                                                                remarks=payment_obj.get_remarks(),
                                                                modify_status=-1,modified_by=-1, modify_ref_id=-1)
               ...
               payment_update=SupplierPayment.objects.using(self._current_app_schema()).filter(id=new_id, entity_id=self._entity_id()).update(modify_status=-1,modified_by=-1,
                                                                modify_ref_id=-1)
               # audit
            ...
            payment_obj = SupplierPayment.objects.using(self._current_app_schema()).get(id=new_id, entity_id=self._entity_id())
            payment_update = SupplierPayment.objects.using(self._current_app_schema()).filter ( id=old_id, entity_id=self._entity_id()).update ( modify_status=-1 , modified_by=-1 ,
                                                                                   modify_ref_id=-1 ,
            ...
            self.audit_function ( payment_update , vendor_id , user_id , req_status , new_id , ModifyStatus.active_in )
               ...
               payment=SupplierPayment.objects.using(self._current_app_schema()).filter(id=old_id, entity_id=self._entity_id()).delete()
               # audit
    ...
    def modification_reject_payment(self, mod_status, old_id, new_id,vendor_id,user_id):
        ...
        if mod_status == ModifyStatus.update:
            ...
            payment_update=SupplierPayment.objects.using(self._current_app_schema()).filter(id=old_id, entity_id=self._entity_id()).update(modify_ref_id=-1)
            # audit
            self.audit_function(payment, vendor_id, user_id, req_status, new_id, ModifyStatus.delete)
            self.audit_function(payment_update, vendor_id, user_id, req_status, old_id, ModifyStatus.update)
        elif mod_status == ModifyStatus.create:
            ...
            payment=SupplierPayment.objects.using(self._current_app_schema()).filter(id=old_id, entity_id=self._entity_id()).update(modify_ref_id=-1)
            # audit
    ...
    def paymentactiveflagservice(self,data):
        paymentupdate = SupplierPayment.objects.using(self._current_app_schema()).filter(id=data.id, entity_id=self._entity_id()).update(is_active=data.is_active)
        if paymentupdate:
            payment=SupplierPayment.objects.using(self._current_app_schema()).get(id=data.id, entity_id=self._entity_id())
    ...
    def fetch_paymentvendor(self,request, vys_page,user_id,vendor_id):
        queue_arr= SupplierPayment.objects.using(self._current_app_schema()).filter(supplierbranch__vendor_id=vendor_id, entity_id=self._entity_id()).values('id')
        condition = None
        for vendor in queue_arr:
            logger.info(str(vendor))
            if condition is None:
    ...
    def supplierpayment(self,branch_id,paymode_id):
        condition = Q(supplierbranch_id=branch_id) & Q(paymode_id=paymode_id) &Q(entity_id=self._entity_id()) &Q(is_active=1)
    ...
    def supplierpaymentacc(self,branch_id,paymode_id,account_no,emp_id):
        condition = Q(supplierbranch_id=branch_id) & Q(paymode_id=paymode_id) & Q(entity_id=self._entity_id()) &Q(is_active=True)
        print(account_no)
        if (account_no is '') or (account_no is None):
            pass
        else:
            condition &= Q(account_no=account_no)
        payment = SupplierPayment.objects.using(self._current_app_schema()).filter(condition)
        list_length = len(payment)
        paymentdata = NWisefinList()
        if list_length > 0:
            for payment in payment:
                ...
                payment_data.set_bank_id(api_serv.get_bank(payment.bank_id,emp_id))
                payment_data.set_branch_id(api_serv.get_bankbranch(payment.branch_id,emp_id))
                payment_data.set_account_type(payment.account_type)
                payment_data.set_account_no(payment.account_no)
                payment_data.set_beneficiary(payment.beneficiary)
                payment_data.set_remarks(payment.remarks)
                ...
                payment_data.set_modify_status(payment.modify_status)
                payment_data.set_is_active(payment.is_active)
                payment_data.set_is_delete(payment.is_delete)
                paymentdata.append(payment_data)
        return paymentdata


    def fetch_creditpayment(self, branch_id,emp_id):
        try:
            payments = SupplierPayment.objects.using(self._current_app_schema()).filter(id__in=branch_id, entity_id=self._entity_id())
            pay_list=NWisefinList()
