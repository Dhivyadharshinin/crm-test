...
from jvservice.models import JVAudit
...
class JVAuditService(NWisefinThread):
    ...
    def create_audit(self, audit_obj):
        try:
            audit_var = JVAudit.objects.using(self._current_app_schema()).create(ref_id=audit_obj.ref_id,
    ...
    def fetch_audit_list(self, user_id):
        auditlist = JVAudit.objects.using(self._current_app_schema()).all()
    ...
    def fetch_audit(self, audit_id, user_id):
        try:
            audit_var = JVAudit.objects.using(self._current_app_schema()).get(id=audit_id,entity_id=self._entity_id())
            audit_data = JVAuditResponse()
            audit_data.set_id(audit_var.id)
    ...
    def delete_audit(self, audit_id, user_id):
        audit = JVAudit.objects.using(self._current_app_schema()).filter(id=audit_id,entity_id=self._entity_id()).delete()
        if audit[0] == 0:
            error_obj = NWisefinError()
            error_obj.set_code(ErrorMessage.INVALID_AUDIT_ID)
            error_obj.set_description(ErrorDescription.INVALID_AUDIT_ID)
