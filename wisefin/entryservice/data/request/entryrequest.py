import json
from entryservice.util.entryutil import ModuleRefType
...
class EntryRequest:
    ...
    def __init__(self, entry_obj):
        ...
        if 'refid' in entry_obj:
            self.refid=RefType.get_val(entry_obj['refid'])
        ...
        if 'vendor_type' in entry_obj:
            self.vendor_type = entry_obj['vendor_type']
        if 'branch_code' in entry_obj:
            self.branch_code = entry_obj['branch_code']
